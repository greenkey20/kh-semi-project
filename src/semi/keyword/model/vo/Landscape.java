...
//2022.1.21(금) 15h5
public class Landscape extends TravelSpot {
	// 부모클래스로부터 추가되는 필드 없음 -> 생성자도 추가적으로 만들 필요 없음 -> 나의 질문 = toString도 오버라이딩할 필요 없나?
	...
	// 2022.1.28(금) 14h10 추천 여행지 1곳 db 랜덤 조회 결과를 CATEGORY/travelType별 객체/자료형으로 담고자 해서, 아래 생성자 추가-> 2022.2.3(목) 13h15 AREA 추가
	public Landscape() {
		super();
	}
	
	public Landscape(int tsNo, String category, String name, String description, String address, String phone,
	...
	// 2022.1.30(일) 0h45 여행지 관리 페이지에 여행지 목록 띄우기 위해 db로부터 여행지 목록을 받아오며, 아래 생성자 추가 -> 2022.1.30(일) 3h5 area 필드 추가 -> 2022.2.1(화) 21h30 해당 여행지의 총 방문 횟수도 보여주고 싶어서 visitCount 매개변수 추가
	public Landscape(int tsNo, String category, String name, String address, String phone, int price, String area, int visitCount) {
		super(tsNo, category, name, address, phone, price, area, visitCount);
	}
	
	// 2022.1.31(월) 13h30 여행지 관리 페이지에서 여행지 추가/등록 처리를 위해, 아래 생성자 추가
	public Landscape(String category, String name, String description, String address, String phone, int price,
